...
class AudioNotifier extends StateNotifier<AudioState> {
  ...
  AudioNotifier(this._player, this._dataSource, this._ref) : super(const AudioState()) {
    _setupListeners();
  }
  ...
  Future<void> playChapter(int chapterId, {int? reciterId}) async {
    ...
    final url = await _dataSource.fetchChapterAudioUrl(chapterId, apiIds.chapterApiId);
    if (url != null) {
      try {
  ...
  Future<void> playVerse(int chapterId, String verseKey, {int? reciterId}) async {
    ...
    final url = await _dataSource.fetchVerseAudioUrl(verseKey, apiIds.verseApiId);
    if (url != null) {
      try {
        await _player.setUrl(url);
        await _player.setSpeed(state.playbackSpeed);
...
final audioProvider = StateNotifierProvider<AudioNotifier, AudioState>((ref) {
  ...
  return AudioNotifier(player, dataSource, ref);
});
