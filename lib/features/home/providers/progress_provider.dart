...
class ProgressNotifier extends StateNotifier<ProgressState> {
  ...
  void updateProgress(int chapterId, String verseKey, int totalVerses) {
    ...
    if (newStats.lastReadDate != today) {
      ...
      final newStreak = newStats.lastReadDate == yesterday
    ...
    final completed = newProgress.values.where((p) => p.versesRead >= p.totalVerses).length;
    final totalRead = newProgress.values.fold<int>(0, (sum, p) => sum + p.versesRead);
    newStats = newStats.copyWith(
  ...
  ReadingProgress? getChapterProgress(int chapterId) => state.progress[chapterId];

  ReadingProgress? getLastReadChapter() {
    if (state.progress.isEmpty) return null;
    return state.progress.values.reduce(
        (a, b) => a.lastReadAt > b.lastReadAt ? a : b);
  }

  double getOverallProgress() {
    if (state.progress.isEmpty) return 0;
    final total = state.progress.values.fold<int>(0, (s, p) => s + p.versesRead);
